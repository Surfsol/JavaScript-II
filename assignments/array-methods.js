...
for (i=0; i < runners.length; i++){
	var first = Object.values(runners[i])[1];
	var last = Object.values(runners[i])[2];
	full.push(first + ' ' + last);
}

console.log(full)


const newlist = runners.map((person) => person.first_name + ' ' + person.last_name);
console.log(newlist)
*/
let fullName =[];
...
runners.map((element) =>{
	var comp = element.company_name;
	company_list.push(comp);
	})
console.log(company_list.sort())

// Problem 2
// find companies with multiple participants

var counts = {};
company_list.forEach(function(x) { counts[x] = (counts[x] || 0)+1; });
console.log(counts)

// Problem 3
//list runners from company with most runners
skinix_runners = []
skinix_runners = runners.filter(x => (x.company_name === 'Skinix'))
console.log(skinix_runners)

// Find out total donations from company
skinix_donations = skinix_runners.reduce(function(accumulator, item){
	return accumulator + item.donation;
}, 0); // starts from 0
console.log(skinix_donations)